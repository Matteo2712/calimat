...
- Problema: nel countdown, i beep dei soli ultimi 3 secondi non davano un preavviso sufficiente per prepararsi. Soluzione: doppio beep ravvicinato di preavviso configurabile (10/15/20s, default 15s) in Impostazioni, distinto dai beep finali; niente pulsante di test dedicato, solo la scelta con il valore attivo evidenziato.
- Problema: la spunta di fine esercizio nello storico era binaria (fatto/non fatto) e non rifletteva se le reps/FIR erano state completate per intero, e la domanda manuale "hai completato tutte le serie?" era ridondante quando avevamo già dati automatici (repClean/fir attivi). Soluzione: stato a 3 livelli calcolato automaticamente a fine sessione — verde (perfect) = serie e reps/FIR tutti al target, arancione (partial) = serie complete ma reps/FIR sotto target, rosso (red) = serie non tutte fatte; la domanda manuale resta solo come fallback quando repClean/fir sono disattivati (nessun dato automatico disponibile). Storico e stats mostrano "fatte/dovute" (reps o somma FIR) invece del formato NxM, verde/rosso in base al raggiungimento del target.
